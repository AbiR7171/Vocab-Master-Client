...
const ManageUser = () => {
  ...
  const handleAdmin = (user) => {
    ...
    axios
      .patch(
        `https://vocab-master-server-new.vercel.app/singleUser/users/admin/${user._id}`
      )
      .then((data) => {
        console.log(data);
  ...
  const handleDelete = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `https://vocab-master-server-new.vercel.app/singleUser/users/${user._id}`
          )
          .then((data) => {
            console.log(data);
        ...
        <table className="table table-zebra container mx-auto px-20 font-Sec mt-10">
          ...
          <tbody>
            {users.map((user, index) => {
              return (
                <tr className="text-gray-400 text-1xl">
                  <th>{index + 1}</th>
                  <td>{user.name}</td>
                  <td>
                    {" "}
                    <img
                      src={user.image}
                      className="w-16 h-16 rounded-lg hover:w-52 hover:h-44  duration-700"
                      alt=""
                    />
                  </td>
                  <td>{user.email}</td>
                  ...
                  <td>
                    <button
                      ...
                      className="bg-red-800 p-4 rounded-md text-white"
                    >
                      DELETE
                    </button>
                  </td>
                  <td>
                    <button
                      ...
                      className="bg-green-800 p-4 rounded-md text-white"
                    >
                      ADMIN
                    </button>
                  </td>
